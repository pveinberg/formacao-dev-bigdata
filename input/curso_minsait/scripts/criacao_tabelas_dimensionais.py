...
print(">> Carregamento de Python libs ...")
...
print(">> Criação da sessão spark ...")
spark = SparkSession.builder.master("local[*]").enableHiveSupport().getOrCreate()



print(">> Definição de variáveis da aplicação ...")
...
print(">> Criação dos dataframes a partir do Hive ...")
...
print(">> Criação dos espaços para tratar campos e criar modelo dimensional ...")
...
print(">> Criação de stage ...")
df_stage = spark.sql(sql)

# Criação dos Campos Calendario

print(">> Criação dos campos de calendário ...")
df_stage = (df_stage
...
print(">> Criação de chaves do modelo dimensional ...")
...
print(">> Criação de modelo dimensional (dimensões) ...")
...
def salvar_df(_df, _file):
    ...
    try:
        ...
        _df.coalesce(1).write            .format("csv")            .option("header", True)            .option("delimiter", ";")            .mode("overwrite")            .save(f"{HDFS_BASE_DIR}/gold/{_file}/")
...
print(">> Processando as dimensões para o servidor local ....")
